...
public class OpenSearchAccessAuditsService extends org.apache.ranger.AccessAuditsService {
    private static final Logger LOGGER = LoggerFactory.getLogger(OpenSearchAccessAuditsService.class);
    ...
    public VXAccessAuditList searchXAccessAudits(SearchCriteria searchCriteria) {
        RestHighLevelClient client              = openSearchMgr.getClient();
        ...
        try {
            response = openSearchUtil.searchResources(searchCriteria, searchFields, sortFields, client, openSearchMgr.index);
        } catch (IOException e) {
            LOGGER.warn("OpenSearch query failed: {}", e.getMessage());
        ...
        try {
            docs = openSearchUtil.fetch(client, openSearchMgr.index, response.getHits().getHits());
        } catch (IOException e) {
            LOGGER.warn("OpenSearch fetch failed: {}", e.getMessage());
            ...
            if (!hiveQueryVisibility && isHive) {
                ...
                if ("grant".equalsIgnoreCase(accessType) || "revoke".equalsIgnoreCase(accessType)) {
                    ...
                    if (requestData != null) {
                        ...
                        LOGGER.warn("Error in request data of audit from openSearch. AuditData: {}", vXAccessAudit);
                    }
                }
            }
